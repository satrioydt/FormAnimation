...
containers.forEach(container =>  {
    ...
    input.addEventListener('focus', () => {
        if(!input.value){
            tl.fromTo(line, {attr: {d: start}}, {attr: {d: end}, ease: "power2.out", duration: 0.5})
            tl.to(line, {attr: {d: start},  ease: "elastic.out(2, 0.4)"}, '<50%')
            tl.to(placeholder, {top: -15, left: 0, scale: 0.7, duration: 0.5, ease: "power2.out"}, '<')
        }
    })
});

// Part of the text minimize and go up
...
// Fungsi buat masukin email atau phone number (bisa searching di internet)
...
// Colorize function. Function buat ganti warna kalo isinya benar/salah
...
// Animasi check box
  ...
  const tl2 = gsap.timeline({defaults: {duration: 0.5, ease: "power2.out"}})
  ...
  checkbox.addEventListener('click', () => {
    if(checkbox.checked){
        ...
        tl2.to('.checkbox-label', {color: '#6F4726'}, '<')
    } else {
...
button.addEventListener('click', (e) => {
    e.preventDefault()
    tl3.to('.contact-right, .contact-left', {y: 30, opacity: 0, PointerEvent: 'none'})
    tl3.to('form', {scale: 0.8}, '<')
    tl3.fromTo('.submitted', {opacity: 0, y: 30}, {opacity: 1, y: 0})
// Hand waving
    gsap.set('#hand', {transformOrigin: 'left'})
    gsap.fromTo('#hand', {rotation: 0, y: 0}, {rotation: -10, y: 2, ease: "elastic.out(2, 0.3),", duration: 3.5, delay: 1})
})